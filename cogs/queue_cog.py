...
class QueueCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.db = bot.db
        self._processing_lobby: set[int] = set()
    ...
    def _get_lobby_channel(self, guild: discord.Guild):
        return discord.utils.get(
            guild.voice_channels, name=config.LOBBY_CHANNEL_NAME
        )
    ...
    async def on_voice_state_update(
        ...
        just_joined_lobby = (
            ...
            and after.channel.id == lobby.id
            and (before.channel is None or before.channel.id != lobby.id)
        )
        ...
        if just_joined_lobby and not self._is_registered(guild.id, member.id):
            ...
            try:
                await member.send(
                    "Чтобы встать в лобби на матч, сначала зарегистрируйся на"
                    " сервере: используй команду /register и укажи свой ник в"
                    " Standoff 2."
                )
    ...
    async def _check_lobby(self, guild: discord.Guild, lobby):
        ...
        registered = [
            m for m in members_in_lobby if self._is_registered(guild.id, m.id)
        ]
        ...
        if len(registered) >= config.QUEUE_SIZE:
            ...
            try:
                ...
                # Отправка анонса в указанный текстовый канал
                match_channel = self.bot.get_channel(config.MATCH_CHANNEL_ID)
                if match_channel:
                    mentions = " ".join([m.mention for m in selected])
                    await match_channel.send(
                        f"🎮 **Матч найден!** Участники:\n{mentions}"
                    )
    ...
    @app_commands.command(
        name="lobbystatus", description="Кто сейчас в голосовом лобби"
    )
    async def lobbystatus(self, interaction: discord.Interaction):
        lobby = self._get_lobby_channel(interaction.guild)
        if lobby is None:
            await interaction.response.send_message(
                f'Голосовой канал "{config.LOBBY_CHANNEL_NAME}" не найден на'
                " сервере.",
                ephemeral=True,
            )
            return
        ...
        if not members:
            await interaction.response.send_message(
                f"В лобби ({lobby.mention}) сейчас никого нет."
            )
            return
